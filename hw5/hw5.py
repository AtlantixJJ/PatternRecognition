...
def derivative_t(data, t_i, theta_i, N):
    ...
    for x in data:
        ...
        f = sum(fi)#sum([t_i[i] * fi[i] for i in range(N)])
        for i in range(N):
            dldt[i] += fi[i]/f
...
def do_MLE(data, N, mus=None, sigmas=None, name="raw"):
    ...
    thetas = []
    ls = []
    init_mu = np.array([[5., 5., 5.], [5., 5., 5.], [5., 5., 5.]])
...
# do MLE
ts, mus, sigmas, thetas = do_MLE(data, 2, name="raw")
...
### N = 3
N = 3
mus1, sigmas1 = KMeans(data, N)

# do MLE
ts2, mus2, sigmas2, thetas2 = do_MLE(data, N, name="raw")

# do MLE inited
ts3, mus3, sigmas3, thetas3 = do_MLE(data, N, mus1, sigmas1, name="init") 