...
filename = "futuresData/0-20170823-night.log"

print("Get dataset")
dic = lib.get_dataset(filename)

print("Classify")

model = "SMOOTHLINEAR"
if model == "SMOOTHLINEAR":
    reg, train_X, test_X, train_Y, test_Y = train.linear_fit(dic["A1"])
    train.show_result_linear(reg, test_X, dic["A1"]['meanPrice200'][train.WIN_SIZE+train_X.shape[0]:-train.DEP_LEN], st=100, ed=1000)
    train.testacc_linear(reg, test_X, test_Y)

elif model == "LINEAR":
    reg, train_X, test_X, train_Y, test_Y = train.linear_fit(dic["A1"])
    train.show_result_linear(reg, test_X, dic["A1"]['meanPrice200'][train.WIN_SIZE+train_X.shape[0]:-train.DEP_LEN], st=100, ed=1000, smooth=False)
    train.testacc_linear(reg, test_X, test_Y, smooth=False)

elif model == "LOGISTIC":
    reg, train_X, test_X, train_Y, test_Y = train.linear_classify(dic["A1"])
    train.testacc_logistic(reg, test_X, test_Y)